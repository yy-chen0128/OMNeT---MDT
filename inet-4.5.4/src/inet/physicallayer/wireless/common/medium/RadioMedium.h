...
class INET_API RadioMedium : public cSimpleModule, public cListener, public IRadioMedium
{
  ...
  //MDT
  public:
      /** Transiently register the given transmission, evaluate reception for given receiver, and remove it.
       *  Returns true if the receiver would successfully receive the transmission. */
      virtual bool testReception(const IRadio *transmitter, const ITransmission *transmission, const IRadio *receiver);
