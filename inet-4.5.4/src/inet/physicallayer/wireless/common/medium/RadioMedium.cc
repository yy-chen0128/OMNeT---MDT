...
//MDT
bool RadioMedium::testReception(const IRadio *transmitter, const ITransmission *transmission, const IRadio *receiver)
{
    // transiently add
    addTransmission(transmitter, transmission);
    // reuse internal logic to compute reception (this uses computed arrivals, interference, snir)
    bool ok = isReceptionSuccessful(receiver, transmission, IRadioSignal::SIGNAL_PART_WHOLE);
    // cleanup
