...
SpatialGrid::SpatialGrid(const Coord& voxelSizes, const Coord& constraintAreaMin, const Coord& constraintAreaMax)
{
    ...
    //throw cRuntimeError("constraintArea = %f %f %f %f %f %f", constraintAreaMax.x,constraintAreaMin.x,constraintAreaMax.y,constraintAreaMin.y,constraintAreaMax.z,constraintAreaMin.z);
...
unsigned int SpatialGrid::computeGridVectorLength() const
{
    ...
    for (unsigned int i = 0; i < 3; i++){
        //throw cRuntimeError("numVoxels= %d %d %d", numVoxels[0],numVoxels[1],numVoxels[2]);
        if (numVoxels[i] != 0)
